...
- **Merge PR & Ticket Sync:** Once approved and CI is green, merge the PR into `main` using **Squash and Merge** (`merge_method: "squash"`). The squash commit title MUST be formatted as `NN: <ticket title> (#<pr_number>)`. Update `README.md`, `ARCHITECTURE.md`, `docs/SDD.md` if applicable, set ticket `**Status:** done`, re-run `python3 docs/tickets/gen-index.py`, and sync tickets. Only after merge is complete are subagents terminated.
