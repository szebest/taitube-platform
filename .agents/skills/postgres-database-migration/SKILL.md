...
description: |
  Use this skill for planning, testing, and safely executing PostgreSQL schema migrations — especially on production data or shared databases.
  Trigger when the user asks to: test a migration before production; add/remove/rename columns on a live table; change a column type without downtime; add or drop indexes, constraints or foreign keys on large tables; understand which ALTER TABLE operations lock the table; roll back a failed migration; plan a zero-downtime migration; fork a database to test safely.
  Keywords: migration, schema change, ALTER TABLE, zero downtime, lock, AccessExclusiveLock, concurrent index, rollback, backfill, deploy.
  Covers lock-level reference for common DDL, safe migration patterns, fork-based testing, zero-downtime column changes, index/constraint addition, backfill strategies, validation, rollback planning.
license: Apache-2.0
metadata:
  upstream: timescale/pg-aiguide skills/postgres-database-migration
  modification: description shortened to fit the 1024-character spec limit; body unchanged
---
