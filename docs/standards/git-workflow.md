...
2. Re-generate the index in the same PR: `python3 docs/tickets/gen-index.py`. CI runs it with `--check` (`tests/architecture/gen-index.test.ts`), so a stale index fails the PR.
