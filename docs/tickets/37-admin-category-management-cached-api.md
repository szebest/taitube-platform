...
- [x] Database migration adding `categories` table (`id` UUIDv7, `slug` text unique, `name` text not null, `description` text, `icon_url` text, `sort_order` integer default 0, `is_active` boolean default true, `created_at`, `updated_at`).
- [x] Foreign key relation / indexing prepared for video categorization (nullable `category_id` references `categories.id` on `videos`).
- [x] `CategoryRepositoryPort` defined in `@taitube/core/repositories/category-repository.port.ts` and domain entity in `@taitube/core/domain/category.ts`.
- [x] Modular PostgreSQL implementation in `adapters/postgres/repositories/postgres-category-repository.ts` (<= 250 lines).
- [x] In-memory test double in `adapters/in-memory/repositories/in-memory-category-repository.ts` with `.clear()` encapsulation.
- [x] L1/L2 Cache Service in `adapters/redis/category-cache.service.ts`:
...
- [x] `GET /v1/categories` public endpoint (no auth required):
...
- [x] Admin endpoints (`POST /v1/admin/categories`, `PATCH /v1/admin/categories/:id`, `DELETE /v1/admin/categories/:id`):
...
- [x] Multi-instance test: Invalidation on Node instance A purges in-memory L1 cache on Node instance B via Pub/Sub.
