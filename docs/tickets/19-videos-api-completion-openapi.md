...
## Acceptance criteria
- [x] `GET /videos?cursor&limit&status` keyset-paginated on `(created_at, id)`, stable under concurrent inserts, scoped to the caller; `nextCursor` opaque.
- [x] `PATCH /videos/:id` with stale `version` → 409 `VERSION_CONFLICT`; `visibility` transitions allowed `private↔unlisted↔public`.
- [x] `/docs` serves OpenAPI 3.1 + Scalar UI; every §6.1 endpoint present with request/response schemas and problem+json error responses listing possible `code`s.
- [x] Contract test: a table derived from SDD §6.1 is compared against the generated spec (fails on drift either way).
- [x] Response `progress.byRendition` and `renditions[]` populated from DB for a `PROCESSING` video.
...
## Open questions
- Public vs signed playback URLs (PRD OQ-2) — MVP public via CDN; note in docs. Resolved: Public CDN delivery at `${CDN_BASE_URL}/videos/{id}/hls/master.m3u8` documented in OpenAPI description and route schemas.

## Definition of Done
- [x] AC green; SDD §6 and the spec agree.
