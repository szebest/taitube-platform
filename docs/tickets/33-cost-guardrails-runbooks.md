...
## Acceptance criteria
- [ ] `R2ClassABudget` fires in a test by lowering the threshold; Storage & Cost dashboard shows Class A/B ops, Neon CU-h (from Neon API or manual panel), Grafana Cloud series count.
- [ ] Raw retention and generation purge observed in R2 (objects disappear after the configured windows).
- [ ] Each runbook: trigger, dashboards to open, diagnosis steps, remediation commands (`kubectl`, Bull Board actions, admin API calls), verification, and "how to prevent next time"; each alert's `runbook_url` resolves.
- [ ] A dry run: hand `worker-stuck.md` to someone/an agent with no context and they resolve a simulated stuck job.
...
## Definition of Done
- [ ] Five runbooks merged; alerts link to them; PRD §11 mitigations marked implemented.
