...
## Acceptance criteria
- [ ] Schedulers exist with the ids/crons from SDD §9.8; restarting the API twice leaves exactly one of each (`getJobSchedulers()`).
- [ ] Test with shortened windows via env: an upload left `UPLOADING` → `ABANDONED` and its multipart aborted (`ListMultipartUploads` empty); an `UPLOADED` video whose probe job was deleted from Redis → re-enqueued and processed to `READY`.
- [ ] `PROCESSING` video with no running step and no waiting jobs for > threshold → `FAILED('ORPHANED')` + `dlq_entries` row.
- [ ] `DELETE /videos/:id` → `DELETED`, objects under `raw/` and `public/videos/{id}/` gone after the purge run (paginated delete tested with > 1 000 objects); row hard-deleted afterwards.
- [ ] Old generation prefix removed after a re-process (16) once the new master is live.
- [ ] All actions are CAS/`SKIP LOCKED` — two housekeeping workers running concurrently do not double-abort or double-delete (test with two in-process workers).
...
## Definition of Done
- [ ] AC green; `docs/runbooks/worker-stuck.md` drafted (orphan handling).
