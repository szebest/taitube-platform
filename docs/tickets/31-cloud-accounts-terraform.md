...
## Acceptance criteria
- [x] Checklist in `docs/runbooks/cloud-accounts.md` covering every row of SDD §15.3 with what to click and which env var each output feeds.
- [x] `terraform plan/apply` idempotent; state stored locally + documented remote option; providers `cloudflare` and `hcloud` pinned.
- [x] R2: two buckets, lifecycle (raw 7 d, abort multipart 1 d), custom domain `cdn.<domain>` proxied; presigned PUT + GET verified with the `STORAGE_E2E_R2=1` test from 05/11.
- [x] Tunnel token + Access policy created; `cloudflared` config committed for 32.
- [x] Hetzner CAX11 (or CX23) created with SSH key only, firewall allowing outbound + SSH from your IP; cost note ≈ €6–7/mo incl. IPv4 and VAT; Oracle path documented with the June-2026 A1 limits.
- [x] `infra/k8s/overlays/cloud/secrets.enc.yaml` (SOPS + age) holds every 🔒 value from `.env.example` §16.8 and the provider sections; decrypt instructions in README.
...
## Definition of Done
- [x] Resources exist; secrets encrypted in repo; checklist complete.
