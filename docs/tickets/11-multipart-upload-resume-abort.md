...
## Acceptance criteria
- [x] Part size = `clamp(ceil(size/1000), 8 MiB, 64 MiB)`; ≤ 10 000 parts; batches of ≤ 100 URLs; URLs expire in 15 min.
- [x] Reference client uploads a 4 GB sparse/synthetic file with concurrency 4; killed at ~50 % and restarted, it resumes from `ListParts`, completes, and the video reaches `READY` (using the `l30` fixture as the source for realism, or a padded file for size).
- [x] `complete` with wrong/missing ETags → 422 with a clear code; after success `HeadObject` size equals declared size, else `REJECTED` and the object deleted.
- [x] `DELETE` → `AbortMultipartUpload`, upload `ABORTED`, video `ABANDONED`; `GET` after that → 410/404 with `UPLOAD_NOT_OPEN`.
- [x] API RSS stays flat (< 300 MB) throughout — the API never buffers file bytes.
- [x] Works unchanged against R2 when `STORAGE_E2E_R2=1` (opt-in test, documents any R2-specific behaviour).
...
## Definition of Done
- [x] AC green; reference client documented; OpenAPI updated.
