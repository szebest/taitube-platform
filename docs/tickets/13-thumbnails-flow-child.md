...
| Issue | [#13](https://github.com/szebest/taitube-platform/issues/13) |
| Size | S–M |
...
## Acceptance criteria
- [x] `s60` → poster + 12-frame sprite (2 rows) + VTT with 12 cues of 5 s; `m10` → 120 frames; VTT validated by a parser test.
- [x] Thumbnail child runs concurrently with transcodes (start times overlap in `processing_steps`).
- [x] Forced thumbnail failure (test flag) → `package` still runs, video `READY`, `poster_key` null, step `FAILED` with code; `renditions` unaffected.
- [x] Test page displays poster and hover preview.
