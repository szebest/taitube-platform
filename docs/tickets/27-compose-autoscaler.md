...
| Issue | [#27](https://github.com/szebest/taitube-platform/issues/27) |
| Size | S |
...
## Acceptance criteria
- [x] Config per stage (min, max, threshold, cooldown) via a small JSON/env; dry-run flag prints intended scale actions.
- [x] Burst of 20 uploads on compose → transcode services scale to max, drain, scale back to min after cooldown; log of actions committed as an example.
- [x] Never scales a service below `min` while it has active jobs (reads `active` state).
...
## Definition of Done
- [x] Tool documented in README "Autoscaling without Kubernetes".
