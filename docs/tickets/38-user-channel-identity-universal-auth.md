...
| Blocks | 39, 40, 41, 42, 43, 44, 46, 47, 49, 50, 56, 72, 76, 77, 78 |
...
- [x] Database migration extending/creating channels and users:
...
- [x] JIT (Just-In-Time) user & channel provisioning hook in Fastify auth middleware: if a verified JWT contains subject sub not yet in DB, inserts user and default channel with handle derived from email/sub.
- [x] Domain entity Channel and ChannelRepositoryPort defined in @taitube/core.
- [x] PostgresChannelRepository in adapters/postgres/repositories/postgres-channel-repository.ts (<= 250 lines).
- [x] InMemoryChannelRepository in adapters/in-memory/repositories/in-memory-channel-repository.ts.
- [x] API Endpoints:
...
- [x] Unit & integration tests via app.inject() verifying JIT provisioning, profile update, handle uniqueness check, and unauthenticated public channel retrieval.
- [x] OpenAPI 3.1 documentation for all user/channel endpoints.
