...
6. Workers do not write `bullmq_queue_jobs`; after one job and an idle period, the KEDA trigger query returns 0 for that queue. Proof: spec on the stage metrics plus the k3d run named in the PR. **Done in 88d: `stages.module.test.ts` finds no `bullmq_queue_jobs` sample after a job. The k3d query is in the PR (see *Decided in 88d*).**
...
- FFmpeg in CI is the static `ffmpeg-static` b6.1.1 build, checksum-verified and cached by version like kustomize. `cache-apt-pkgs-action` restored `ffmpeg` without `libblas.so.3`, so every FFmpeg spec failed with exit 127; apt without a cache costs ~20 s a job.
...
2. FFmpeg and the generated fixtures are cached, keyed on the generator source and `manifest.json`; e2e generates only the fixtures its reduced set uses. Proof: CI log shows a cache hit on a second run. **Done in 88d: `.github/actions/setup-ffmpeg` (a pinned, checksum-verified static FFmpeg 6.1.1 cached by version) and `.github/actions/video-fixtures` (keyed on `gen-video/src/**` and `manifest.json`; e2e generates `--only s2`). Cache hits in the PR.**
3. `docker buildx` uses `cache-from`/`cache-to: type=gha` in `ci.yml` and `images.yml`, with a pnpm store cache mount and a pinned turbo instead of `npx turbo`. Proof: a second run's build step under 45 s. **Done in 88d: e2e bakes both images side by side (`docker-bake.hcl`), `images.yml` builds each image once for the scan and once to push, all on `type=gha` scopes they share; the Dockerfiles install a pinned turbo and mount the pnpm store.**
...
6. On a code PR, CI wall-clock is **under 6 minutes** and `unit` under 2m30, from 10m07 and 4m17. Proof: `ci-shape` asserts that the `timeout-minutes` along the longest `needs` chain sum to 6 or less, which holds it; `gh run view --json jobs` of the last delivery PR pasted. **Done in 88d: `ci-shape` sums the chain (build 2 + e2e-smoke 4 = 6). Timings in the PR.**
7. The FFmpeg-heavy specs use the `s2`/`s15` fixtures: `keyframe-alignment.test.ts` under 10 s (39.9 s today), `segment-streaming-uploader` under 5 s (21.5 s). Proof: vitest timings in the CI log. **Done in 88d: `keyframe-alignment` runs `vfr` and `s2` (half-second segments) with the three renditions in parallel, `segment-streaming-uploader` streams `s2`; timings in the PR.**
8. `pnpm test:architecture` stays under 4 s with the type-aware assertions added, because they share one `ts.Program`. Proof: timing pasted. **Done in 88d: `repo-files.ts` lists the index once per process instead of a `git ls-files` per call, and fixture programs share the parsed `lib.*.d.ts`; timing in the PR.**
