...
2. Every repository and cache contract runs from the per-file spec of each implementation; `repositories.contract.test.ts` is gone. A deliberately drifted double (off-by-one in the feed rank) fails both the double's spec and the Postgres spec. Proof: fixture run pasted in the PR. **Done in 88e: `repositories.contract.test.ts` is gone; each of the 12 in-memory and 12 Postgres repository files has a spec that runs its `describeXRepositoryContract`, and the cache, storage, multipart, job queue and flow producer ports have contracts run from each double's spec and each real adapter's. The drifted-rank fixture (in-memory feed rank off by one) fails six feed tests in `in-memory-video-repository.test.ts`; the Postgres spec stays green, because it tests Postgres and nothing of the double reaches it: the shared assertion in `video-feed.contract.ts` is what catches the drift (output in the PR). The contracts found six defects the specs had hidden (see *Decided in 88e*).**
...
11. The nightly load smoke is green for 14 consecutive nights before the ticket closes, and the PR names the root cause of the three 100% `http_req_failed` nights. The `presign` threshold is measured against a baseline recorded on the same runner class, not a fixed 200 ms. Proof: `gh run list --workflow load-smoke.yml` pasted. **Partly done in 88e, the 14 nights are what is left. Root cause of the three 100% nights (09-19, 09-20, 09-21): the workflow minted its token with `TOKEN=$(pnpm -w exec tsx tools/dev-token/src/cli.ts mint || echo <token>)`; the path did not exist, so the captured output was pnpm's error text plus the fallback token on a second line, and every request failed in k6 with `invalid header field value for "Authorization"` before it left the runner. 52d209e had already replaced it with a pre-baked token. The two later failures (09-22, 09-24) were the fixed `p(95)<200` on slow runners that were slow for everything (complete p95 255 ms, 170 iterations against 250 on a normal night). The bar is now measured on the same runner: a one-VU baseline step records presign p95 and p99, and the run is held to six times each (first run: baseline p95 26.6 ms, bar 160 ms, measured 56 ms; a baseline that records no p95 or p99 fails the step). The nightly runs on `main`, so the 14 nights can only start counting once this merges; the ticket stays open on them.**
