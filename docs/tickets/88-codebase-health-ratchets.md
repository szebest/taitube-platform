...
**ACs**
1. Under `NODE_ENV=production`, `GET /.well-known/jwks.json` is 404, and a token minted by `pnpm dev-token mint --role admin` is 401 on `/admin/dlq`. Proof: `apps/api/src/__tests__/auth-hardening.test.ts`.
