...
- Found while reading, not fixed here: the worker's R2 token reads `raw` only, while `expire-raw` and `purge-deleted` delete from it; `OTEL_TRACES_SAMPLER=parentbased_traceidratio` is read as always-on, so the cloud samples 100 %; the KEDA Redis fallback on `transcode-480p` reads the `wait` list, which a prioritized job never enters. They are behaviour changes, so they are in the PR as follow-ups.
