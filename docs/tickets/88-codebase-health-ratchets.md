...
8. `pnpm test:architecture` stays under 6 s in CI with the type-aware assertions added, because they share one `ts.Program`, and a CI step fails the run when it does not. Proof: the check and its timing. **Done in 88d: 17.3 s -> 5.2-5.4 s in CI (vitest `Duration`, runs 35999524765 and 36000110560), 2.4-3.6 s locally. The target was 4 s; Mateusz moved it to 6 s after the attempts below. `lint-typecheck` runs the suite with the JSON reporter and `.github/actions/budget` fails the job when the report's first start to last end is over 6 s, CI only, so a loaded laptop does not flake it; `ci-shape` asserts the step is there with 6.** `repo-files.ts` lists the index once per process and matches each glob once, `parsed-sources.ts` parses each file once, `start-order` moved to `tests/in-process`, and the suite is two processes side by side: the AST assertions in one thread and the type-aware ones in one fork over one `ts.Program`, which leaves out `lib.dom.d.ts` and caches module resolution. The typed fork is the long pole in CI: building the program and `env-keys-consumed` take 1.9 s there, the other two 0.8 s each, on top of about 1 s importing TypeScript. Tried and reverted: a fork per typed assertion, each with the smallest program it needs (server sources only for `env-keys-consumed` and `promql-labels-emitted`), took 7.64 s in CI (run 36001055967), because four processes on a four-core runner each import TypeScript and build a program. Importing TypeScript lazily saves nothing, since every file in both processes needs it and each process already imports it once.
