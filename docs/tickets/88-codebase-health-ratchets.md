...
- `prioritized` joins the poller's states from one list, `QUEUE_JOB_STATES` in `@vp/core/ports`, which the BullMQ adapter (`getJobCounts(...COUNTED_STATES)`, no cast), the in-memory double and the poller read. BullMQ 6 keeps no `paused` list, since a paused queue leaves its jobs where they were, so both adapters count `paused` as 0 and a paused queue is read through `isPaused`; the in-memory double used to count its jobs twice, as waiting and as paused. Every pipeline job carries a priority, so BullMQ never had a pipeline job in `waiting`; the KEDA queries only ever saw the workers' `active=1`. The in-memory double now counts a job with a priority as `prioritized`, as BullMQ does.
...
5. A PR touching only `**/*.md` or `docs/**` completes CI in under 90 s, with required checks reported. Proof: `ci-shape` asserts the path filter; the run of the first docs-only PR merged after 88d, pasted. **Done in 88d: the path filter is the first step of `build`, whose `code` output every other job gates on, so a docs-only PR reports every required check as skipped. The run of the first docs-only PR can only exist after this merges; see the PR.**
6. On a code PR, CI wall-clock is **under 6 minutes** and `unit` under 2m30, from 10m07 and 4m17. Proof: `ci-shape` asserts that the `timeout-minutes` along the longest `needs` chain sum to 6 or less, which holds it; `gh run view --json jobs` of the last delivery PR pasted. **Done in 88d: `ci-shape` sums the chain (build 2 + e2e-smoke 4 = 6). `unit` 2m16 and 2m26 (runs 35999524765, 36000110560), wall 3m13 and 3m07; the FFmpeg `.deb` cache now hits (its key was empty before), and `unit` no longer installs Bun, which none of its specs run. `timeout-minutes` takes whole minutes, so the unit budget stays 3 and 2m30 is held by the timings, not by a timeout.**
7. The FFmpeg-heavy specs use the `s2`/`s15` fixtures: `keyframe-alignment.test.ts` under 10 s (39.9 s today), `segment-streaming-uploader` under 5 s (21.5 s). Proof: vitest timings in the CI log. **Done in 88d: `keyframe-alignment` runs `vfr` and `s2` (half-second segments) with the three renditions in parallel, `segment-streaming-uploader` streams `s2`: 9.7 s and 2.0 s in CI (run 35992685851).**
8. `pnpm test:architecture` stays under 4 s with the type-aware assertions added, because they share one `ts.Program`. Proof: timing pasted. **Not met in 88d: 17.3 s -> 5.2-5.4 s in CI (vitest `Duration`, runs 35999524765 and 36000110560), 2.4-3.6 s locally.** `repo-files.ts` lists the index once per process and matches each glob once, `parsed-sources.ts` parses each file once, `start-order` moved to `tests/in-process`, and the suite is two processes side by side: the AST assertions in one thread and the type-aware ones in one fork over one `ts.Program`, which leaves out `lib.dom.d.ts` and caches module resolution. The typed fork is the long pole in CI: building the program and `env-keys-consumed` take 1.9 s there, the other two 0.8 s each, on top of about 1 s importing TypeScript. A fork per typed assertion was slower (each builds the program again). What is left to try is a smaller program per assertion (server sources only for the config and PromQL ones).
