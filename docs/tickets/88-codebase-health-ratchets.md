...
8. `pnpm test:architecture` stays under 4 s with the type-aware assertions added, because they share one `ts.Program`. Proof: timing pasted. **Not met in 88d: 17.3 s -> 9.6 s in CI (`lint-typecheck`, run 35993729990), still over 4 s.** `repo-files.ts` lists the index once per process instead of a `git ls-files` per call, fixture programs share the parsed `lib.*.d.ts`, and `test:architecture` runs `--no-isolate` so each worker builds the `ts.Program` once. What is left is real work: the three type-aware assertions check the whole program (about 4 s each on a runner), and a dozen AST assertions each parse all production sources again. Getting under 4 s needs the parsed sources shared across files and `start-order` (which composes both apps) moved out of the suite.
