...
## Acceptance criteria
- [ ] `make k3d-up` creates the cluster and installs KEDA, kube-prometheus-stack, Redis, MinIO, Postgres via Helm with committed values in < 5 min.
- [ ] `make k3d-deploy` applies migrations (Job), API and all worker Deployments; pods run non-root, read-only root FS, `emptyDir` for `/tmp/vp` with `sizeLimit`, resource requests/limits and `terminationGracePeriodSeconds` per stage as in SDD §12.2; `FFMPEG_THREADS` from `resourceFieldRef limits.cpu`.
- [ ] `make smoke` green against the cluster; Grafana in-cluster shows the 22 dashboards via ConfigMaps.
- [ ] `kustomize build overlays/local` and `overlays/cloud` (skeleton) both validate with `kubeconform`.
- [ ] Worker liveness probe uses the heartbeat file; readiness for API hits `/readyz`.
...
## Definition of Done
- [ ] AC green; README "Run on Kubernetes locally".
