...
## Acceptance criteria
- [ ] `/admin/queues` lists all queues in `QUEUES` (including `dlq`, created lazily); unauthenticated → 401; non-admin JWT → 403; `x-admin-token` compared in constant time.
- [ ] Pausing `transcode-720p` from the UI stops new jobs from starting (verify with a queued job); resuming continues.
- [ ] An admin plugin (`requireAdmin`) is reusable by later admin routes (16, 19).
...
## Definition of Done
- [ ] Screenshot in PR; documented in README "Operations".
