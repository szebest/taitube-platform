...
## Acceptance criteria
- [ ] `s60` → `READY` in < 90 s on an 8 vCPU laptop; `master.m3u8` + `720p/index.m3u8` + 10 `.ts` segments exist with the SDD §7 content-types and cache headers; `renditions` row `DONE` with `segment_count=10`, `bytes`, `processing_ms`.
- [ ] Every segment starts with an IDR (verify with ffprobe on two random segments); playlist contains `#EXT-X-INDEPENDENT-SEGMENTS`, `#EXT-X-PLAYLIST-TYPE:VOD`, `#EXT-X-ENDLIST`; `gop = round(2·fps)` derived from probe, not hard-coded.
- [ ] `package` verifies the rendition playlist exists via HEAD before writing the master; master is written **last**; CAS to `READY` happens once; `video_events` contains exactly one `video.ready`.
- [ ] `notify` publishes `{event:'status', data:{status:'READY', playbackUrl}}` on `video:{id}` and `user:{uid}` (subscribe in the test to assert).
- [ ] The test page plays the video from `CDN_BASE_URL` (MinIO anonymous read) and hls.js reports a single level.
- [ ] Progress callbacks from FFmpeg reach `job.updateProgress()` at most every 2 s and `processing_steps.heartbeat_at` advances.
- [ ] FFmpeg exit 137 maps to `TransientError('FFMPEG_OOM')`, timeout to `FFMPEG_TIMEOUT`, `Invalid data found` to `PermanentError('CORRUPT_CONTAINER')` (unit tests with stubbed processes).
...
## Definition of Done
- [ ] Recorded GIF/screenshot of playback in the PR; AC green; README "First video end-to-end".
