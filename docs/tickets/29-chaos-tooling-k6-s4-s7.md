...
| Issue | [#29](https://github.com/szebest/taitube-platform/issues/29) |
| Size | L |
...
## Acceptance criteria
- [x] Tooling: `kill-worker.sh` (k8s + compose modes), toxiproxy compose profile fronting MinIO with latency/timeout/503 toxics, `redis-restart.sh`, `disk-fill.sh`; all documented.
- [x] S4: 50/50 videos `READY`, one `video.ready` each, `segment_count` correct, object audit finds no extras; stalled counter > 0.
- [x] S5: 60 s storage 503 → all READY with visible retries; 15 min outage → DLQ entries, `SystemicFailure` alert observed, queue paused per runbook, replay after recovery succeeds; Redis restart → no lost videos.
- [x] S6: `sse_connections = 5000`, API RSS < 512 MB, p95 latency < 2 s (from `ts` in payload), API restart → clients reconnect and receive `snapshot`; no missed terminal events.
- [x] S7: 4 h at 1 upload/10 s: RSS slope < 5 %/h, `/tmp/vp` empty between jobs, `videos_by_status{PROCESSING}` returns to 0.
- [x] Results and interpretation committed; any newly discovered failure mode gets an SDD §9.5 row and, if needed, a follow-up ticket.
...
## Definition of Done
- [x] S4–S7 pass and are documented; SDD §9.5 updated with observations.
