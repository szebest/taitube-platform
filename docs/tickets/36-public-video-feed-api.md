...
1. **List public videos without any token** — `GET /v1/feed` returns a paginated list of all `visibility=public` + `status=READY` videos, sorted newest first. No auth required.
2. **Fetch a single public video without a token** — `GET /v1/videos/:id` already handles this correctly for `public` and `unlisted` videos but needs a test proving the unauthenticated path.
3. **Subscribe to SSE for a public/unlisted video without a token** — `GET /v1/videos/:id/events` currently calls `requireAuth`; it must accept anonymous connections when the video is `public` or `unlisted`.

The existing `GET /v1/videos` endpoint is **not changed** — it remains a "my videos" endpoint scoped to the authenticated caller. The new `/v1/feed` is a separate, clearly named public browse resource.

## Acceptance criteria

- [ ] `GET /v1/feed` — no `Authorization` header needed — returns `{ items: VideoSummaryView[], nextCursor }` containing only `visibility=public` AND `status=READY` videos, newest first, keyset-paginated on `(created_at, id)` (same cursor format as `/v1/videos`).
- [ ] `GET /v1/feed?limit=N` (1–100, default 20) and `GET /v1/feed?cursor=<opaque>` work correctly.
- [ ] `GET /v1/feed` with an optional valid `Authorization: Bearer <token>` still works (token ignored for scoping — feed is always global public).
- [ ] `GET /v1/videos/:id` with no token returns 200 for `visibility=public` and `visibility=unlisted`; returns 401 for `visibility=private`. *(Existing behaviour — add explicit unauthenticated route test.)*
...
- [ ] Contract test: `GET /v1/feed` present in generated OpenAPI, `security: []`, response schema matches `VideoListResponseSchema`.
- [ ] Unit test: `VideoService.listPublic` returns only `public` + `READY` videos; private/processing/deleted videos are excluded regardless of caller.
- [ ] Integration test via `app.inject()`: anonymous `GET /v1/feed` returns 200; anonymous `GET /v1/videos/:id/events` on a public video returns 200; anonymous `GET /v1/videos/:id/events` on a private video returns 401.
