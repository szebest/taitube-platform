...
readily - which is exactly what `<Can>` and `canReadVideo` already do in `apps/web` today. Neither
side is privileged, and `packages/client/api-client` carries the fixture that proves the client tier
can call these rules.
...
- **A rule returns a verdict, never a presentation.** No message aimed at a user, no status code, no
  ...
  `decideVideoRead` keeps `VIDEO_NOT_FOUND` and `FORBIDDEN` distinct instead of collapsing them the
  way `video-service.ts` used to.
...
- **One rule per file**, named `<thing>.rule.ts`, with a `failures.ts` per resource exporting its
  variants and its union.
...
- **Compose, do not restate.** `decideReact` calls `decideVideoRead` rather than repeating its two
  branches.
- **Every suite runs under node and jsdom.**
