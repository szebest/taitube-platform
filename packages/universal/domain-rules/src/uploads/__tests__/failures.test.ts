...
describe('@vp/domain-rules: upload failures', () => {
  it.each([
    { name: 'uploadNotOpen', failure: uploadNotOpen('u1', 'ABORTED'), code: ErrorCodes.UPLOAD_NOT_OPEN },
    { name: 'uploadExpired', failure: uploadExpired('u1', expiresAt), code: ErrorCodes.UPLOAD_EXPIRED },
    {
  ...
  it('says so when the upload record has no part count at all', () => {
    expect(partManifestMismatch('u1', null, 2).message).toContain('missing its multipart part count');
  });
