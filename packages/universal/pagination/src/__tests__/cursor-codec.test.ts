...
import { Base64UrlCursorCodec, type CursorPayload } from '../cursor-codec';

describe('packages/pagination: Base64UrlCursorCodec', () => {
  ...
  it.each([
    ...
    expect(codec.decode(btoa(raw)).ok).toBe(false);
  });
});
