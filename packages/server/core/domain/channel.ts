...
/**
 * Handles a new channel may claim, most desirable first: the identity's own
 * name, then the same name disambiguated by part of the subject, then numbered
 * variants. Every candidate already satisfies `isValidHandleFormat` and is not
 * reserved, so a caller only has to test availability.
 */
