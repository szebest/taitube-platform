...
describe('packages/config: loadEnvOrExit', () => {
  ...
  it('writes one JSON line naming each invalid key, no stack, and exits 1', () => {
    ...
    expect(log.lines()).toEqual([
      expect.objectContaining({
        ...
        invalid: expect.arrayContaining([expect.stringMatching(/^PORT: /)]),
      }),
    ]);
