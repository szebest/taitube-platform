...
describe('s3 adapter: connection config', () => {
  ...
  it.each([
    ...
    { scenario: 'a cloud endpoint', endpoint: 'https://account.r2.cloudflarestorage.com', expected: false },
  ...
  it.each([
    { scenario: 'a NotFound exception', cause: Object.assign(new Error('x'), { name: 'NotFound' }) },
    { scenario: 'a NoSuchKey exception', cause: Object.assign(new Error('x'), { name: 'NoSuchKey' }) },
  ...
  it.each([
    { scenario: 'a permission error', cause: Object.assign(new Error('x'), { name: 'AccessDenied' }) },
