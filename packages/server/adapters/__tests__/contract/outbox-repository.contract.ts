...
export function describeOutboxRepositoryContract(makeSubject: MakeRepositoriesSubject): void {
  describe('OutboxRepository contract', () => {
    ...
    it('enqueues an unpublished record with a zero attempt count', async () => {
      ...
      expect(await outbox.findById(OUTBOX_IDS.first)).toMatchObject({ kind: 'job.enqueue' });
      expect(await outbox.findById(OUTBOX_IDS.unknown)).toBeNull();
    });
    ...
    it('claims only unpublished records, oldest first, up to the limit', async () => {
      ...
      expect((await outbox.claimBatch(1)).map((r) => r.kind)).toEqual(['first']);

      await outbox.markPublished(OUTBOX_IDS.first);
      expect((await outbox.claimBatch(10)).map((r) => r.kind)).toEqual(['second']);
    });

    it('reports a missing record on publish and attempt bookkeeping', async () => {
      await outbox.enqueue({ id: OUTBOX_IDS.first, kind: 'first', payload: PAYLOAD });

      expect(await outbox.recordAttempt(OUTBOX_IDS.first)).toBe(true);
      expect((await outbox.findById(OUTBOX_IDS.first))?.attempts).toBe(1);
      expect(await outbox.markPublished(OUTBOX_IDS.unknown)).toBe(false);
      expect(await outbox.recordAttempt(OUTBOX_IDS.unknown)).toBe(false);
    });
    ...
    it('prunes nothing while the published records are inside the retention window', async () => {
      ...
      expect(await outbox.prune(7)).toBe(0);
      expect(await outbox.findById(OUTBOX_IDS.first)).not.toBeNull();
    });
    ...
    it('prunes the published records once the retention window has passed', async () => {
      ...
      expect(await outbox.prune(-1)).toBe(1);
      expect(await outbox.findById(OUTBOX_IDS.first)).toBeNull();
      expect(await outbox.findById(OUTBOX_IDS.second)).not.toBeNull();
    });
  });
}
