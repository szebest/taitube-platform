...
- **Schema Definitions:** Drizzle ORM tables (`src/schema.ts`, subpath `@vp/db/schema`, with
  ...
  `video_comments` and `src/view-schema.ts` for `video_views_daily` and `video_view_batches`;
  `drizzle.config.ts` reads all four). `schema.ts` sits near the 10 KB ceiling, so a new table goes in a
  module of its own.
