...
10. **Branch Protection, PR-Only Merges with Required Approval & Green CI in DoD (Strict Barrier):** Direct pushes and direct merges to `main` are strictly forbidden and blocked by GitHub branch protection rules. Every ticket MUST be developed on its own branch (`ticket/NN-slug`) and merged to `main` exclusively through a GitHub Pull Request with at least one required approval and all status checks passing green. The implementor agent pushes the branch and creates the PR, sharing the URL in user chat. The reviewer agent reviews the PR on GitHub, leaving line/summary review comments on the PR. Both implementor and reviewer agents MUST remain alive throughout the review-fix loop. The orchestrator relays feedback via agent messaging; the implementor either applies code fixes (pushing to the branch) or replies explaining technical rationale why a comment is not applicable. Once addressed, the reviewer re-evaluates the PR. Only after the reviewer issues a formal approval on the PR and all CI workflow checks (`lint-typecheck`, `unit`, `unit-bun`, `integration`, `e2e-smoke`) pass green on GitHub Actions may the PR be merged into `main`. Both agents remain alive until the PR is merged.
