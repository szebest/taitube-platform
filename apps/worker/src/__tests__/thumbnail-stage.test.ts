...
describe("Thumbnail Stage as Non-Blocking Flow Child (Ticket 13: AC 1, 2, 3)", () => {
  ...
  async function setupUploadedVideo(
    sourceKey: string,
  ...
  it("AC 1: generates poster, 12-frame sprite, and VTT with 12 cues for s60; uploads with immutable headers", async () => {
    ...
    expect(posterHead?.cacheControl).toBe(
    ...
    expect(spriteHead?.cacheControl).toBe(
      "public, max-age=31536000, immutable",
    );
    ...
    const vttText =
      typeof vttObj === "string"
        ? vttObj
        : new TextDecoder("utf-8").decode(vttObj);
