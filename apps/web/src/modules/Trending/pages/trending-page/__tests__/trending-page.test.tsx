...
describe('apps/web: trending page', () => {
  it('shows the trending feed, not the newest one', async () => {
    ...
    const markup = renderPage(<TrendingPage />, { store });
  ...
  it('shows the spinner while the first page loads', () => {
    expect(renderPage(<TrendingPage />)).toContain('aria-label="Loading"');
  });
  ...
  it.each([
    ...
    expect(renderPage(<TrendingPage />)).toContain(gridIcon);
  });
});
